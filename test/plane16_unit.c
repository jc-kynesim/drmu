...
static const struct { unsigned int w, h; } test_sizes[] = {
    ...
    { 13,  7 },   /* odd × odd, larger */
    {  6,  6 },   /* even, square */
    { 15,  9 },   /* odd, 4:2:0 subsampling edge */
    { 192, 144 }, /* > 128, divisible by 1,2,3,6 */
    { 257, 193 }, /* > 128, odd × odd */
    { 320, 181 }, /* > 128, even × odd */
};
...
test_fmt(const drmu_fmt_info_t * const px, const unsigned int w, const unsigned int h)
{
    ...
    plane16_from_generic(p16, p16_stride, px,
    ...
    for (plane = 0; plane != 4 && px->planes[plane].bpg != 0 && rv == 0; ++plane) {
         ...
         * plane16_from_generic ignores them on write, and plane16_to_generic
...
main(int argc, char *argv[])
{
    ...
    for (unsigned int i = 0; i != sizeof(test_fmts) / sizeof(test_fmts[0]); ++i) {
        ...
        for (unsigned int s = 0; s != sizeof(test_sizes) / sizeof(test_sizes[0]); ++s) {
            const unsigned int w = test_sizes[s].w;
            const unsigned int h = test_sizes[s].h;
            const int rv = test_fmt(px, w, h);
            if (rv != 0) {
                printf("FAIL %s %ux%u\n", drmu_fmt_info_name(px), w, h);
                ++fail;
            } else {
                printf("PASS %s %ux%u\n", drmu_fmt_info_name(px), w, h);
                ++pass;
