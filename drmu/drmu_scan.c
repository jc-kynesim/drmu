                 ...
                 drmu_env_t ** const pDu, drmu_output_t ** const pDoutput)
{
    ...
    *pDoutput = NULL;
    ...
    if ((rv = card_list_scan(&cl, dlog)) != 0)
        return rv;

    rv = -ENOENT;
    for (i = 0; i != cl.n; ++i) {
        const unsigned int cardno = cl.nos[i];
        drmu_env_t * du;
        drmu_output_t * dout = NULL;
        char fname[64];
        int fd;

        drmu_debug_log(dlog, "Try card %u", cardno);

        snprintf(fname, sizeof(fname), CARD_DIR "/" CARD_PREFIX "%u", cardno);
        ...
        if ((du = drmu_env_new_fd(fd, dlog)) == NULL) {
            drmu_debug_log(dlog, "Cannot create du");
            continue;
        }
        ...
        if (drmu_output_add_output(dout, conn_name) == 0) {
